...
function renderJourneyBlock(block: JourneyBlock, options: { primaryColor: string; dense?: boolean }): string {
  ...
  return `
    <section class="journey ${dense ? 'journey--dense' : 'journey--regular'}">
      ...
      ${block.hasStopover && block.stopover ? `<div style="color: ${options.primaryColor}; font-size: ${dense ? 20 : 24}px; font-weight: var(--font-weight-dates); margin-bottom: ${dense ? 12 : 16}px; margin-top: ${dense ? -6 : -8}px;">${escapeHtml(block.stopover)}</div>` : ''}
...
<html lang="pt-BR">
  ...
  <body>
    <div class="canvas" id="alert-preview-canvas">
      ...
      ${payload.footer.hasExtraObservation && payload.footer.extraObservation && theme.footerOverlayLayout.extraObservationLine ? `
      <div
        class="footer-line footer-line--primary"
        data-fit-text data-fit-text-lines="1" data-fit-text-min="10"
        style="left: ${theme.footerOverlayLayout.extraObservationLine.x}px; top: ${theme.footerOverlayLayout.extraObservationLine.y}px; width: ${theme.footerOverlayLayout.extraObservationLine.width}px; color: var(--text-color);"
      >${escapeHtml(payload.footer.extraObservation)}</div>` : ''}
      <div
        class="footer-line footer-line--primary"
        data-fit-text data-fit-text-lines="1" data-fit-text-min="10"
