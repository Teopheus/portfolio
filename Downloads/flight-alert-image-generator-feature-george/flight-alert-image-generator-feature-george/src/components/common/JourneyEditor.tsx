      ...
      <div className="flex flex-col gap-2 pt-1 pb-2">
        <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-zinc-900">
          <input
            ...
            checked={!!block.hasStopover}
            onChange={(event) =>
              onChange({
                ...block,
                hasStopover: event.target.checked,
                stopover: event.target.checked ? block.stopover || '' : block.stopover
              })
            }
          />
          Adicionar informacao de parada
        </label>
        
        {block.hasStopover && (
          <Field label="" hint="Ex: 1 PARADA NO PANAMA">
            <input
              className="w-full rounded-xl border border-zinc-200 bg-zinc-50/50 px-4 py-3.5 text-sm text-zinc-900 transition-all hover:bg-zinc-50 hover:border-zinc-300 focus:border-zinc-900 focus:bg-white focus:outline-none focus:ring-4 focus:ring-zinc-900/10 placeholder:text-zinc-400"
              value={block.stopover || ''}
              onChange={(event) => onChange({ ...block, stopover: event.target.value })}
              placeholder="1 PARADA NO PANAMA"
            />
          </Field>
        )}
      </div>
