              ...
              <div className="flex flex-col gap-2 pb-2">
                <label className="flex items-center gap-2 cursor-pointer text-sm font-medium text-zinc-900">
                  <input
                    ...
                    checked={!!payload.footer.hasExtraObservation}
                    onChange={(event) =>
                      setPayload((current) => ({
                        ...current,
                        footer: {
                          ...current.footer,
                          hasExtraObservation: event.target.checked,
                          extraObservation: event.target.checked ? current.footer.extraObservation || '' : current.footer.extraObservation
                        }
                      }))
                    }
                  />
                  Adicionar observacao extra
                </label>
                
                {payload.footer.hasExtraObservation && (
                  <Field label="" hint="Ex: OBSERVACAO EXTRA AQUI">
                    <input
                      className={inputClassName}
                      value={payload.footer.extraObservation || ''}
                      onChange={(event) =>
                        setPayload((current) => ({
                          ...current,
                          footer: { ...current.footer, extraObservation: event.target.value }
                        }))
                      }
                      placeholder="OBSERVACAO EXTRA AQUI"
                    />
                  </Field>
                )}
              </div>
